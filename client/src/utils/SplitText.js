...
(function(window) {
	...
	var _globals = window.GreenSockGlobals || window,
		...
		_isBeforeWordDelimiter = function(e, root, wordDelimiter) {
			while (e && e !== root) {
				if (e.nextSibling) {
					return e.nextSibling.textContent.charAt(0) === wordDelimiter;
				}
				e = e.parentNode;
			}
			return false;
			...
			if (lines || absolute) {
				for (i = 0; i < l; i++) {
					...
					if (isChild || absolute || (chars && !words)) {
						...
						if (lines) {
							if (node._isSplit || (!chars && isChild) || (words && isChild) || (!words && node.parentNode.parentNode === element && !node.parentNode._isSplit)) {
								curLine.push(node);
								node._x -= paddingLeft;
			...
			for (i = 0; i < l; i++) {
				...
				if (!words && chars) {
					...
					if (node._isSplit) {
						...
						offset = (!node.nextSibling && node.parentNode._next && node.parentNode._next.textContent.charAt(0) === wordDelimiter); //if this is the last letter in the word (and we're not breaking by lines and not positioning things absolutely), we need to add a space afterwards so that the characters don't just mash together
